...
function SignUp() {
  ...
  const { registerOrg, handleSubmitOrg, formState: { errorsOrg } } = useForm({
    resolver: yupResolver(validationSchemaOrg),
  });
...
/*       console.error('Error sending data:', error); */
  ...
  const onSubmitOrg = async (data) => {
    try {
      ...
      const response = await axios.post('https://ghablameh.fiust.ir/api/v1/organizations/register/', formattedDataOrg);
      const accessToken = response.data.tokens.access;
      localStorage.setItem('token', accessToken);
          navigate("/");
    } catch (error) {
